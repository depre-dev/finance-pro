...
export default function Projects() {
  ...
  const getRemainingBudget = (totalBudget: string | number, actualCost: string | number) => {
    return parseFloat(totalBudget.toString()) - parseFloat(actualCost.toString());
  };

  const calculateBudgetUsage = (totalBudget: string | number, actualCost: string | number) => {
    const total = parseFloat(totalBudget.toString());
    const actual = parseFloat(actualCost.toString());
    if (total === 0) return 0;
